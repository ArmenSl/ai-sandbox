...
def init_db():
    SQLALCHEMY_DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///./data/ai_sandbox_PSA_13_Jan_2026.db")
...
async def create_legalrequirement(legalrequirement_data: LegalRequirementCreate, database: Session = Depends(get_db)) -> LegalRequirement:
    ...
    db_legalrequirement = LegalRequirement(
        standard=legalrequirement_data.standard,        legal_ref=legalrequirement_data.legal_ref,        principle=legalrequirement_data.principle,        project_1_id=legalrequirement_data.project_1        )
...
async def bulk_create_legalrequirement(items: list[LegalRequirementCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_legalrequirement = LegalRequirement(
                standard=item_data.standard,                legal_ref=item_data.legal_ref,                principle=item_data.principle,                project_1_id=item_data.project_1            )
            database.add(db_legalrequirement)
            database.flush()  # Get ID without committing
...
async def create_tool(tool_data: ToolCreate, database: Session = Depends(get_db)) -> Tool:
    ...
    db_tool = Tool(
        version=tool_data.version,        licensing=tool_data.licensing.value,        name=tool_data.name,        source=tool_data.source        )
...
async def bulk_create_tool(items: list[ToolCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_tool = Tool(
                version=item_data.version,                licensing=item_data.licensing.value,                name=item_data.name,                source=item_data.source            )
            database.add(db_tool)
            database.flush()  # Get ID without committing
...
def get_paginated_datashape(skip: int = 0, limit: int = 100, detailed: bool = False, database: Session = Depends(get_db)) -> dict:
    ...
    for datashape_item in datashape_list:
        ...
        item_data = {
            ...
            "f_date_ids": [x[0] for x in f_date_ids],            "f_features_ids": [x[0] for x in f_features_ids],            "dataset_1_ids": [x[0] for x in dataset_1_ids]        }
        result.append(item_data)
    return {
        "total": total,
        "skip": skip,
...
async def create_measure(measure_data: MeasureCreate, database: Session = Depends(get_db)) -> Measure:
    ...
    db_measure = Measure(
        value=measure_data.value,        uncertainty=measure_data.uncertainty,        unit=measure_data.unit,        error=measure_data.error,        measurand_id=measure_data.measurand,        observation_id=measure_data.observation,        metric_id=measure_data.metric        )
...
async def bulk_create_measure(items: list[MeasureCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_measure = Measure(
                value=item_data.value,                uncertainty=item_data.uncertainty,                unit=item_data.unit,                error=item_data.error,                measurand_id=item_data.measurand,                observation_id=item_data.observation,                metric_id=item_data.metric            )
            database.add(db_measure)
            database.flush()  # Get ID without committing
...
async def create_model(model_data: ModelCreate, database: Session = Depends(get_db)) -> Model:
    ...
    db_model = Model(
        pid=model_data.pid,        source=model_data.source,        licensing=model_data.licensing.value,        data=model_data.data,        dataset_id=model_data.dataset,        project_id=model_data.project        )
...
async def bulk_create_model(items: list[ModelCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_model = Model(
                pid=item_data.pid,                source=item_data.source,                licensing=item_data.licensing.value,                data=item_data.data,                dataset_id=item_data.dataset,                project_id=item_data.project            )
            database.add(db_model)
            database.flush()  # Get ID without committing
...
async def create_dataset(dataset_data: DatasetCreate, database: Session = Depends(get_db)) -> Dataset:
    ...
    db_dataset = Dataset(
        version=dataset_data.version,        licensing=dataset_data.licensing.value,        dataset_type=dataset_data.dataset_type.value,        source=dataset_data.source,        datashape_id=dataset_data.datashape,        project_id=dataset_data.project        )
...
async def bulk_create_dataset(items: list[DatasetCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_dataset = Dataset(
                version=item_data.version,                licensing=item_data.licensing.value,                dataset_type=item_data.dataset_type.value,                source=item_data.source,                datashape_id=item_data.datashape,                project_id=item_data.project            )
            database.add(db_dataset)
            database.flush()  # Get ID without committing
...
async def bulk_create_feature(items: list[FeatureCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_feature = Feature(
                min_value=item_data.min_value,                max_value=item_data.max_value,                feature_type=item_data.feature_type,                date_id=item_data.date,                features_id=item_data.features,                project_id=item_data.project            )
            database.add(db_feature)
            database.flush()  # Get ID without committing
...
async def create_observation(observation_data: ObservationCreate, database: Session = Depends(get_db)) -> Observation:
    ...
    db_observation = Observation(
        name=observation_data.name,        description=observation_data.description,        whenObserved=observation_data.whenObserved,        observer=observation_data.observer,        eval_id=observation_data.eval,        dataset_id=observation_data.dataset,        tool_id=observation_data.tool        )
...
async def bulk_create_observation(items: list[ObservationCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_observation = Observation(
                name=item_data.name,                description=item_data.description,                whenObserved=item_data.whenObserved,                observer=item_data.observer,                eval_id=item_data.eval,                dataset_id=item_data.dataset,                tool_id=item_data.tool            )
            database.add(db_observation)
            database.flush()  # Get ID without committing
